...
import pandas as pd
import numpy as np
from numkit.timeseries import apply_histogrammed_function, regularized_function
...
def publication_CRMSD_plot(RMSD_cdf, protomer, band_frames=100, xlim=None):
    ...
    #mavg_OF = stdev_histogrammed_function(times, RMSD_OF, bins=10000)
...
def summary_plot(da, selection, alignment, xlims=[0.5,2.5]):
    ...
    axs[0,0].set_ylabel("IF RMSD ($\AA$)")
    axs[1,0].set_ylabel("density")
    
    axs[2,0].set_ylabel("OCC RMSD ($\AA$)")
    axs[3,0].set_ylabel("density")
    
    axs[4,0].set_ylabel("OF RMSD ($\AA$)")
